...
class RandomPlayer:
    def get_ai_move(self, board, current_player, green_type=None, red_type=None):
        valid_moves = []
        #all valid moves
        for row in range(9):
            for col in range(7):
                ...
                if piece and piece.color == current_player:
                    ...
                    for move_row, move_col in moves:  
                        if is_valid_position(move_row, move_col, color=piece.color):
                            valid_moves.append(((row, col), (move_row, move_col)))
        return random.choice(valid_moves) if valid_moves else None

class NegamaxPlayer:
    def get_ai_move(self, board, current_player, depth=3, green_type=None, red_type=None):
        best_moves = []
        ...
        for row in range(9):
            for col in range(7):
                ...
                if piece and piece.color == player:
                    ...
                    rank = adjusted_rank(piece.type, row, col, board)
                    score += rank * 10
...
class AlphaBetaPlayer:
    ...
    def get_ai_move(self, board, current_player, depth, green_type=None, red_type=None):
        best_moves = []
        best_score = float('-inf')
        alpha = float('-inf')
        beta = float('inf')
